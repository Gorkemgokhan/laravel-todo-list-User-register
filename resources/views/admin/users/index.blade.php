<x-app-layout>
    <div style="width: 768px;"class="max-w-full mx-auto mt-10 px-4 py-8 text-gray-500">
        <h1 class="text-xl uppercase text-gray-500 mb-4">Bütün kullanıcılar</h1>
        @include('partials.notifications')
        <table class="w-full table-auto mt-4">
            <thead>
                <tr class="text-left bg-gray-600 text-white uppercase text-base">
                    <th class="px-2 py-2">Ad Soyad</td>
                    <th class="px-2 py-2">email</td>
                    <th class="px-2 py-2">Kademesi</th>
                ...
                <tr class="bg-gray-200 hover:bg-gray-100">
                    ...
                    <td class="px-2 py-4 text-white">
                        @if($user->id !== Auth::user()->id)
                            <a href="{{route('admin.users.edit', $user)}}" class="px-2 py-1 bg-gray-400">Rolü Düzenle</a>
                            <form action="{{route('admin.users.destroy', $user)}}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button class="px-2 py-1 bg-red-400"onclick="return confirm('bu görevi silmek istediğinizden emin misiniz?')">Sil</button>
                            </form>
                        @endif
                    </td>
                </tr>
